        ...
        body: SafeArea(
          child: Center(
            child: Column(
              ...
              children: [
                Text('"$groupId" ${cm['notFound'] ?? ''}', style: AmaniTheme.titleStyle.copyWith(fontSize: 18)),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(tFormat(cm['wordCount'] ?? '', {'count': group.words.length}), style: AmaniTheme.bodyStyle.copyWith(fontSize: 12, color: AmaniColors.textSecondary)),
                      ],
                    ),
                  ),
                  GestureDetector(
                    onTap: () => speech.speak(tFormat(cm['introSpeak'] ?? '', {'titre': groupTitle}), lang),
                    child: Container(
                      width: 40,
                      height: 40,
              ...
              child: ListView(
                ...
                children: [
                  Container(
                    ...
                    child: Row(
                      children: [
                        const AmaniMascot(pose: AmaniPose.demonstration, size: AmaniSize.small),
                        const SizedBox(width: 14),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(cm['introTitle'] ?? '', style: AmaniTheme.titleStyle.copyWith(fontSize: 14)),
                              const SizedBox(height: 2),
                              Text(cm['introBody'] ?? '', style: AmaniTheme.bodyStyle.copyWith(fontSize: 12, color: AmaniColors.textSecondary)),
                            ],
                          ),
                        ),
                      ],
                  ...
                  for (final word in group.words) ...[
                    _WordCard(word: word, lang: lang, onSpeak: () => speech.speak(word.text(lang.name), lang)),
                          ...
                          child: Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                Flexible(
                                  child: Text(
                                    tFormat(cm['practiceGroup'] ?? '', {'titre': groupTitle}),
                  ...
                  Row(
                    ...
                    children: [
                      if (prevGroup != null)
                        _NavPill(label: prevGroup.title[lang.name] ?? '', leading: true, onTap: () => context.go('/cours/mots/${prevGroup.id}'))
                      else
                        const SizedBox(),
                      if (nextGroup != null)
                        _NavPill(label: nextGroup.title[lang.name] ?? '', leading: false, filled: true, onTap: () => context.go('/cours/mots/${nextGroup.id}'))
                      else
                        const SizedBox(),
                    ],
                  ),
...
class _WordCard extends StatelessWidget {
  ...
  const _WordCard({required this.word, required this.lang, required this.onSpeak});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: AmaniColors.textPrimary.withValues(alpha: 0.1)), boxShadow: const [BoxShadow(color: Color(0x14000000), blurRadius: 8)]),
      child: Row(
        children: [
          Expanded(
            child: letters.isEmpty
                ? Text(text, style: const TextStyle(fontFamily: kBalooFontFamily, fontWeight: FontWeight.w800, fontSize: 20, color: AmaniColors.textPrimary))
                : SingleChildScrollView(
                    scrollDirection: Axis.horizontal,
                    child: Row(
                      children: [
                        for (final letter in letters) ...[
                          LetterTraceCell(letter: letter, size: 48, isActive: false, given: true),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Icon(CupertinoIcons.speaker_2_fill, size: 18, color: Color(0xFF2D6BBF)),
            ),
          ),
        ],
      ),
...
class _NavPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final text = Text(label, style: TextStyle(fontFamily: kBalooFontFamily, fontWeight: FontWeight.w800, fontSize: 13, color: filled ? Colors.white : AmaniColors.textPrimary));
    return GestureDetector(
      onTap: onTap,
      child: Container(
        constraints: const BoxConstraints(maxWidth: 170),
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        decoration: BoxDecoration(color: filled ? const Color(0xFF4A90E2) : AmaniColors.surface, borderRadius: BorderRadius.circular(999), border: filled ? null : Border.all(color: AmaniColors.textPrimary.withValues(alpha: 0.1))),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: leading ? [icon, const SizedBox(width: 6), Flexible(child: text)] : [Flexible(child: text), const SizedBox(width: 6), icon],
        ),
      ),
    );
  }
